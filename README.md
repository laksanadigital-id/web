...
- `index.html`: hanya headline lalu katalog (`#katalog`): satu kartu per aplikasi berisi pratinjau aplikasi (gambar di dalam bingkai jendela, dari `images/<produk>/cover.png`), nama, satu baris deskripsi, harga dengan penanda "sekali bayar", dan satu tombol perincian selebar kartu. Home page tidak memuat apa pun yang spesifik ke satu produk (tidak ada daftar fitur, marquee kemampuan, strip KPI, atau kartu peran); semua itu hanya ada di halaman produk. Dijaga oleh `test_catalog_grid` dan pemeriksaan grid di `tests/browser_smoke.py`. Navigasi header halaman katalog memuat tautan Katalog, tautan Request Website, tombol konsultasi, dan pemilih tema; tautan ke halaman produk cukup lewat kartu katalog. Tautan "Katalog" dan tombol hero diarahkan ke `#katalog-heading` (pembungkus judul) supaya judul "Katalog aplikasi" tetap terlihat setelah digulir, bukan ke `#katalog` yang menandai grid. Setelah katalog ada section layanan website (`#layanan-website`) untuk jasa pembuatan website: judul tengah, subjudul, tiga kartu penjelasan berikon, dan satu CTA WhatsApp; polanya sama dengan section halaman produk (tanpa eyebrow). Latarnya memakai warna khusus `bg-layanan-soft` / `dark:bg-layanan-soft-dark`, tint biru lembut yang didefinisikan sebagai token `--color-layanan-soft` dan `--color-layanan-soft-dark` di `assets/tailwind.css` supaya bisa disetel dari satu tempat.
- `apotek.html`: halaman produk Laksana Apotek tanpa breadcrumb: hero berisi headline, satu baris hook harga, nominal sekali bayar, satu kalimat penjelas, dan sepasang tombol beli (tanpa kartu terpisah), strip marquee, grid 8 tangkapan layar, perincian fitur per bagian kerja, spesifikasi, kartu peran (ikon garis, bukan ilustrasi), dan FAQ "Info penting".
...
- Level desain: **frontend level creative** (aturan level normal tetap berlaku sebagai dasar). Satu skema warna aksen: **blue**; netral `slate`. Hal yang wajib dipertahankan saat menyunting:
  - Hero memakai display font Fredoka (`font-display`, bukan Inter) dengan gradient text; teks hero lainnya (paragraf, tombol) tetap Inter.
  - Tanpa efek fade-in masuk: hook `data-reveal` dan animasi `animate-rise`/`animate-pop` sudah dihapus, semua elemen tampil langsung pada nilai akhirnya (tidak ada stagger `[transition-delay:...]`). Dijaga oleh `test_no_entrance_fade_effects`.
...
- JavaScript bersama di `assets/site.js` hanya berisi pemilih tema, efek header saat digulir, counter `data-counter`, modal zoom `data-shot-open`/`data-shot-modal`, parallax, dan cursor glow. Inisialisasi tema sebelum render ada di `assets/theme.js`. Tanpa JavaScript semua konten tetap tampil pada nilai akhirnya, dan thumbnail tetap berupa tautan ke berkas gambarnya.
